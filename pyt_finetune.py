...
def add_augmentation(x_train, input_file_name):
    ...
    file_name = data_dir + language_translated_aug_dir + 'augmented_' + file_name
    csv_file = file_name
    augtexts = []
    translations = []
...
def train_and_evaluate_model(model_name='distilbert-base-uncased', input_file_name='data_dir/data/ADHD.tsv', seed=42):
    ...
    if model_text == 'Pico_sentences':
        ...
        input_file_name_pico = data_dir +pico_dir + 'pico_' + input_file_name_pico
        dir_path = Path(input_file_name_pico)
        if not dir_path.exists():
            logging.info(f'not running for dataset {input_file_name} , no pico data for this dataset')
            return -1
        else:
            return 0
        if not dir_path.exists():
